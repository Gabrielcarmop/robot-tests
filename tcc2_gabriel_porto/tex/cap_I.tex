...
\par  Depois dessa fase inicial, o projeto será implementado em um sistema real para testar sua eficácia em um ambiente de produção. Os testes foram configurados para rodar automaticamente sempre que um novo \textit{commit} é feito no repositório do projeto no GitHub, ou em horários definidos, com o uso do \textit{GitHub Actions}. Quando uma falha é detectada, o sistema gera automaticamente uma \textit{issue} no próprio \textit{GitHub}, contendo \textit{prints} da tela, \textit{logs} e uma descrição resumida com a ajuda de uma inteligência artificial que interpreta o erro.
